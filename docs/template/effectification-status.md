...
The manifest phase adds target generated ownership for
`packages/template-core/src/generated/confectManifest.ts` and the
`rtk pnpm confect:manifest` command. Until that phase lands, runtime headless
projection still derives from the canned `templateRegistry`.
