...
| Use Confect/Effect contracts                  | mechanical: `check:convex` codegen drift diff, `@confect/test` contract tests, generated-manifest parity tests, semantic `check:confect-contracts`; pin-only remains only for static docs and package-matrix guards |
...
| Hosted app works                     | mechanical: Playwright smoke + accessibility + visual baselines (`smoke:hosted:*`), static build smoke; the smoke suite asserts live workflow runs stream from the deployed Convex backend |
